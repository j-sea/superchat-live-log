...
if (typeof window.superchatLiveLogLoaded === 'undefined') {
    ...
    function startSuperchatLiveLogs() {
        if (chatItems !== null) {
            ...
            superchatsExpireLabel.style.marginRight = '0.5rem';
            ...
            maxHistorySizeInput.style.margin = '0.2rem 0.5rem 0.2rem 0.2rem';
            ...
            showHistoryLabel.style.marginRight = '0.5rem';
            ...
            showExtensionLabel.style.marginRight = '0.5rem';
            ...
            showExtensionCheckbox.addEventListener('change', function(event){
                if (showExtensionCheckbox.checked) {
                    ...
                    creditsLegend.textContent = 'credits (ver. 1.4.5)';
                }
                else {
                    extensionLabel.style.display = 'none';
                    creditsLegend.textContent = 'credits (superchat live log ver. 1.4.5)';
                }
            ...
            scrolledUpWarningLabel.style.marginRight = '0.5rem';
            ...
            saveSettingsLabel.style.marginRight = '0.5rem';
            ...
            thanksPeopleDiv.textContent = `immense thanks goes to ${['AxsDeny','Christina','Jake'].reduce((acc, curr, index, arr) => {
                if (arr.length === 0) {
                ...
                if (liveLogBoxContainer.style.position === 'absolute') {
                    ...
                    if (currentChildIndex > 0) {
                        if (currentSuperchatExpired) {
                            ...
                            const expiredSuperchat = currentSuperchats[currentChildIndex];
            ...
            function showVisibilityEye() {
                if (liveLogBoxContainer.style.position === 'absolute') {
                    const appearingHeight = liveLogBox.clientHeight;
                    const itemScroller = chatPanel.querySelector('#item-scroller');
                    const originalScrollTop = itemScroller.scrollTop;
            ...
            pinAnimator.addPinExpirationListener(function(superchatIndex){
                if (currentChildIndex !== superchatIndex) {
                    ...
                    // if (expiredSuperchat.classList.contains('paid-message')
                    //     || expiredSuperchat.classList.contains('membership-item')) {
                    //     const messageNode = expiredSuperchat.querySelector('.sll-message');
                    //     if (messageNode !== null) {
                    //         messageNode.textContent = 'superchat expired';
                    //         // messageNode.style.color = 'rgba(255, 255, 255, 0.7)';
                    ...
                    // const userIcon = expiredSuperchat.querySelector('.sll-user-icon');
                    // const userName = expiredSuperchat.querySelector('.sll-user-name');
                    // const sticker = expiredSuperchat.querySelector('.sll-sticker');
                    // if (userIcon !== null) {
                    //     userIcon.remove();
                    // }
                    // if (userName !== null) {
                ...
                function copySuperchatToLiveLog(addedNode, tagName) {
...

                    {
                        ...
                        if (tagName === 'yt-live-chat-paid-message-renderer') {
                            ...
                            const messageHTML = addedNode.data.hasOwnProperty('message') ? `
                                <div
                                    class="sll-message"
                                    style="
                                        padding:8px 16px;
                                    ...
                                    "
                                >
                                    ${addedNode.data.message.runs.reduce((prev, current, index, arr) => {
                                        if (current.hasOwnProperty('text')) {
                                            return prev + current.text;
                                        }
                                        else if (current.hasOwnProperty('emoji')) {
                                            return prev + `<img src="${current.emoji.image.thumbnails[0].url}" alt="${current.emoji.image.accessibility.accessibilityData.label}" height="24" width="24" style="margin:-1px 2px 1px; vertical-align:middle;" />`;
                                        }
                                    }, '')}
                                </div>
                            ` : addedNode.data.hasOwnProperty('footer') ? `
                                <div
                                    class="sll-message"
                                    style="
                                        padding:8px 16px;
                            ...
                            clonedNodeHeaderImage.setAttribute('src', addedNode.data.authorPhoto.thumbnails[0].url);
                            ...
                            divImg.setAttribute('src', addedNode.data.authorPhoto.thumbnails[0].url);
                            div.append(divImg);
                            ...
                            imgUserIcon.setAttribute('src', addedNode.data.authorPhoto.thumbnails[0].url);
                            ...
                            `);
                            imgUserIcon.setAttribute('class', 'sll-user-icon');
                            imgUserIcon.setAttribute('src', addedNode.data.header.liveChatSponsorshipsHeaderRenderer.authorPhoto.thumbnails[0].url);
                            divHeader.append(imgUserIcon);
