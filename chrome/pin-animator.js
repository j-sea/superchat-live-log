...
function createPinAnimator(canvas, canvasContainer) {
  ...
  function redraw(dt) {
    ...
    if (pins.length !== 0) {
      ...
      hoveringPin = -1;
      for (let i = pins.length - 1, iSize = pins.length; i >= 0; i--) {
        const pin = pins[i];
        if (pin.color !== 'invisible') {
          ...
          if ((currentPinIndex < 32 || i >= currentPinIndex - 32)
              && (iSize - 1 - currentPinIndex < 32 || i < currentPinIndex + 32 + 1))
          {
  ...
  return {
    ...
    removePin: function(index) {
      if (index <= currentPinIndex) {
        if (lastAnimationX === targetXPos) {
          currentPinIndex--;
          targetXPos = lastAnimationX = context.lineWidth * 2.25 * currentPinIndex;
        }
        else {
